...
export const useSettingsStore = create<SettingsState>((set, get) => ({
  ...
  setKeyStorage: (mode: KeyStorage) => {
    ...
    if (mode === 'none') {
      ...
      saveToSession({ keyStorage: 'none', apiKey: '', apiBaseUrl: currentUrl, apiModel: currentModel });
    } else if (mode === 'session') {
      localStorage.removeItem(LOCAL_KEY);
      saveToSession({ apiKey: currentKey, keyStorage: 'session', apiBaseUrl: currentUrl, apiModel: currentModel });
    } else if (mode === 'local') {
      sessionStorage.removeItem(SESSION_KEY);
      saveToLocal({ apiKey: currentKey, keyStorage: 'local', apiBaseUrl: currentUrl, apiModel: currentModel });
    }
